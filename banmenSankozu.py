...
def list_tograph_sanko(filename,
    ...
    pdf=PdfPages(savedir+'/参考図_'+filename_2[-1].replace('.bod','').replace('.kif','')+".pdf")
    #グラフの体裁設定
    fig = plt.figure() 
    ...
    #縦番号
    ax.text(10.38, 9.5, '一', ha='center',va='center',rotation=0,fontsize=7,weight='bold')
    ax.text(10.38, 8.5, '二', ha='center',va='center',rotation=0,fontsize=7,weight='bold')
    ax.text(10.38, 7.5, '三', ha='center',va='center',rotation=0,fontsize=7,weight='bold')
    ax.text(10.38, 6.5, '四', ha='center',va='center',rotation=0,fontsize=7,weight='bold')
    ax.text(10.38, 5.5, '五', ha='center',va='center',rotation=0,fontsize=7,weight='bold')
    ax.text(10.38, 4.5, '六', ha='center',va='center',rotation=0,fontsize=7,weight='bold')
    ax.text(10.38, 3.5, '七', ha='center',va='center',rotation=0,fontsize=7,weight='bold')
    ax.text(10.38, 2.5, '八', ha='center',va='center',rotation=0,fontsize=7,weight='bold')
    ax.text(10.38, 1.5, '九', ha='center',va='center',rotation=0,fontsize=7,weight='bold')
    #盤上の点
    ...
    #持ち駒
    sente_mochigoma='☗'+'先手'+' '+sente_mochigoma.replace("二","２").replace("三","３").replace("四","４").replace("五","５").replace("六","６").replace("七","７").replace("八","８").replace("九","９")
    koute_mochigoma='☖'+'後手'+' '+koute_mochigoma.replace("二","２").replace("三","３").replace("四","４").replace("五","５").replace("六","６").replace("七","７").replace("八","８").replace("九","９")
    #先手持ち駒の描画
    senteX=0.6
    ...
    for k in range(len(dan)):
        for i in range(len(dan[k])):
            if dan[k][i][0]=='v':
                komamoji=str(dan[k][i][1])
                moji=komafont[komamoji]
                rot=180
            elif dan[k][i][0]=='・':
                moji=str(dan[k][i]).replace('・','')
                rot=0
            else:
                komamoji=str(dan[k][i])
                moji=komafont[komamoji]
                rot=0
            
            if dan[k][i][0]=='v' and dan[k][i][1]=='杏':
                moji=komafont['杏']
                ax.text(i+1.5,y,moji, ha='center',va='center',rotation=rot,fontsize='8.5',fontweight=1000,family='syogi')
                ...
                if dan[k][i][0]=='v' and dan[k][i][1]=='龍':
                    ax.text(i+1.5,y,moji, ha='center',va='center',rotation=rot,fontsize='9',fontweight=1000,family='syogi')
